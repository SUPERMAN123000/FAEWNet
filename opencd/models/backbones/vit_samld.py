...
class SALayer(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y_0 = self.avg_pool(x).view(b, c)
        y_1 = self.max_pool(x).view(b, c)
        y = y_0 + y_1
        y1 = self.fc1(y)
        y2 = self.fc2(y)
        y3 = self.fc3(y)
...
class SobelConv(nn.Module):
    def __init__(self,channels):
        ...
        self.DACA = DACA(channels)
    def forward(self, x):
        DFT = torch.fft.fft(torch.fft.fft(x, dim=-1), dim=-2).real
        sum = DFT + self.DACA(x)
        return sum
...
class ViTSAM_DAFA(BaseBackbone):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.out_indices:
                # (B, H, W, C) -> (B, C, H, W)
                x_reshape = x.permute(0, 3, 1, 2)+tmp
